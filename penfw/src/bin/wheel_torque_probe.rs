...
//! Wheel-side torque probe.
//!
//! Lay the pendulum on its side so the body is mechanically supported, then
//! flash this binary. It calibrates the hall/electrical mapping, applies short
//! fixed-voltage torque pulses, and prints wheel speed and wheel acceleration.
//! That lets us estimate the actuator's effective `tau / I_wheel` without the
//! normal balancing dynamics dominating the measurement.
...
const PHASE_SEARCH_TARGET_WHEEL_SPEED_DPS: f32 = -360.0;
...
const PRE_PULSE_SETTLE_LOOPS: u32 = 120;
const PULSE_ON_LOOPS: u32 = 16;
const PULSE_REST_LOOPS: u32 = 48;
const PULSE_LEVELS_V: [f32; 3] = [1.2, 2.4, 3.6];
...
struct WheelObserver {
    last_unwrapped_angle_deg: Option<f32>,
    filtered_speed_dps: f32,
    filtered_accel_dps2: f32,
}
...
fn main() -> ! {
    ...
    write_line(&mut serial, "wheel_torque_probe start");
    ...
    let _ = writeln!(
        serial,
        concat!(
            "vlimit={:.1}V pulse_on={}loops pulse_rest={}loops levels=[{:.1},{:.1},{:.1}]V ",
            "pwm={}Hz\r"
        ),
        VOLTAGE_LIMIT_V,
        PULSE_ON_LOOPS,
    ...
    write_line(&mut serial, "wheel_torque_probe enabling in 1s");
    delay.delay_millis(1_000);
    ...
    write_line(
        &mut serial,
        "lay body on its side, keep it fixed, and watch wheel_ddot during pulse_on",
    );
    ...
    let mut stage_loop = 0u32;
    let mut print_divider = 0u32;
    let mut last_announced_stage = usize::MAX;

    loop {
        let current = current_sensor.read();
        let stage = probe_stage(stage_index);

        if stage_index != last_announced_stage {
            let _ = writeln!(
                serial,
                "stage={} label={} uq={:+.2}V loops={}\r",
                stage_index,
                stage.label,
                stage.uq_v,
                stage_duration_loops(stage_index),
            );
            let _ = serial.flush();
            last_announced_stage = stage_index;
        }
        ...
        if let Ok(measurement) = hall.read_measurement() {
            ...
            let (ua_v, ub_v, uc_v) = if stage.uq_v.abs() > 0.0 {
                simplefoc_sine_pwm_phase_voltages(
                    stage.uq_v,
            ...
            if print_divider == 0 {
                let _ = writeln!(
                    serial,
                    concat!(
                        "label={} loop={:>3} hall={:>7.2}deg unwrap={:>8.2}deg ",
                        "elec={:>7.2}deg uq={:+4.2}V wheel_dot={:+8.2}dps ",
                        "wheel_ddot={:+9.2}dps2 uvw=[{:>4.2},{:>4.2},{:>4.2}]V ",
        ...
        stage_loop += 1;
        if stage_loop >= stage_duration_loops(stage_index) {
            stage_loop = 0;
            stage_index = (stage_index + 1) % total_probe_stages();
        }
        delay.delay_millis(CONTROL_PERIOD_MS);
    }
}

fn total_probe_stages() -> usize {
    1 + PULSE_LEVELS_V.len() * 4
}

fn stage_duration_loops(stage_index: usize) -> u32 {
    if stage_index == 0 {
        PRE_PULSE_SETTLE_LOOPS
    } else if (stage_index - 1) % 2 == 0 {
        PULSE_ON_LOOPS
    } else {
        PULSE_REST_LOOPS
    }
...
fn probe_stage(stage_index: usize) -> PulseStage {
    if stage_index == 0 {
        return PulseStage {
            uq_v: 0.0,
            label: "settle",
    ...
    let pulse_block = (stage_index - 1) / 4;
    let substage = (stage_index - 1) % 4;
    let uq_v = PULSE_LEVELS_V[pulse_block];
    match substage {
        0 => PulseStage {
            uq_v: uq_v,
            label: "pulse_pos",
...
impl WheelObserver {
    fn new() -> Self {
        Self {
            last_unwrapped_angle_deg: None,
            filtered_speed_dps: 0.0,
            filtered_accel_dps2: 0.0,
        }
    }

    fn observe(&mut self, raw_angle_deg: f32) -> WheelObservation {
        let unwrapped_angle_deg = match self.last_unwrapped_angle_deg {
            Some(previous) => unwrap_near(previous, raw_angle_deg),
            None => raw_angle_deg,
        };
        let instant_speed_dps = self
            .last_unwrapped_angle_deg
            .map(|previous| (unwrapped_angle_deg - previous) / dt_s())
            .unwrap_or(0.0);
        let previous_filtered_speed_dps = self.filtered_speed_dps;
        self.filtered_speed_dps = 0.7 * self.filtered_speed_dps + 0.3 * instant_speed_dps;
        let instant_accel_dps2 = (self.filtered_speed_dps - previous_filtered_speed_dps) / dt_s();
        self.filtered_accel_dps2 = 0.8 * self.filtered_accel_dps2 + 0.2 * instant_accel_dps2;
        self.last_unwrapped_angle_deg = Some(unwrapped_angle_deg);

        WheelObservation {
            unwrapped_angle_deg,
            speed_dps: self.filtered_speed_dps,
            accel_dps2: self.filtered_accel_dps2,
        }
    }
}
...
fn refine_torque_phase_offset(
    ...
    let target_sign = signum_nonzero(PHASE_SEARCH_TARGET_WHEEL_SPEED_DPS);
